...
export class Utils {
    ...
    public static TO_STRING_DATE_WITHOUT_TIMEZONE(date: Date): string | null {
        if (!date) {
            return null;
        }
    ...
    public static GET_DATE_PERIOD(values: any[] | null): ITimelineDatePeriodBase {
        ...
        values.forEach((value: any) => {
            ...
            if (endDate === undefined || (date !== undefined && date > endDate)) {
                endDate = date;
            }
        });

        if (!startDate || !endDate) {
            throw new Error("Could not determine startDate or endDate from values.");
        }
    ...
    public static ARE_BOUNDS_OF_SELECTION_AND_AVAILABLE_DATES_THE_SAME(timelineData: ITimelineData): boolean | undefined {
        if (!timelineData.currentGranularity) {
            return false;
        }
     ...
     */
    public static GET_START_SELECTION_DATE(timelineData: ITimelineData): Date | undefined {
        return timelineData.currentGranularity && timelineData.selectionStartIndex !== undefined
            ? timelineData.currentGranularity.getDatePeriods()[timelineData.selectionStartIndex].startDate
            : undefined;
    }
     ...
     * @param timelineData The TimelineData which contains all the date periods
     */
    public static GET_END_SELECTION_DATE(timelineData: ITimelineData): Date | undefined {
        return timelineData.currentGranularity && timelineData.selectionEndIndex !== undefined
            ? timelineData.currentGranularity.getDatePeriods()[timelineData.selectionEndIndex].endDate
            : undefined;
    }
     ...
     * @param timelineData The TimelineData which contains all the date periods
     */
    public static GET_END_SELECTION_PERIOD(timelineData: ITimelineData): ITimelineDatePeriod | undefined {
        return timelineData.currentGranularity && timelineData.selectionEndIndex !== undefined
            ? timelineData.currentGranularity.getDatePeriods()[timelineData.selectionEndIndex]
            : undefined;
    }
    ...
    public static GET_CELL_COLOR(
        ...
        const endSelectionDate = Utils.GET_END_SELECTION_DATE(timelineData);
        const startSelectionDate = Utils.GET_START_SELECTION_DATE(timelineData);
        const inSelectedPeriods: boolean = startSelectionDate !== undefined
            && endSelectionDate !== undefined
            && dataPoint.datePeriod.startDate >= startSelectionDate
            && dataPoint.datePeriod.endDate <= endSelectionDate;
    ...
    public static IS_GRANULE_SELECTED(dataPoint: ITimelineDataPoint, timelineData: ITimelineData): boolean {
        const endSelectionDate = Utils.GET_END_SELECTION_DATE(timelineData);
        const startSelectionDate = Utils.GET_START_SELECTION_DATE(timelineData);
        return startSelectionDate !== undefined
            && dataPoint.datePeriod.startDate >= startSelectionDate
            && endSelectionDate !== undefined
            && dataPoint.datePeriod.endDate <= endSelectionDate;
    }
    ...
    public static TIME_RANGE_TEXT(timelineData: ITimelineData): string {
        ...
        const startSelectionDateArray: (string | number)[] = timelineData.currentGranularity
            .splitDateForTitle(Utils.GET_START_SELECTION_DATE(timelineData) ?? new Date(0));

        const endSelectionPeriod = Utils.GET_END_SELECTION_PERIOD(timelineData);
        const endSelectionDateArray: (string | number)[] = timelineData.currentGranularity
            .splitDateForTitle(endSelectionPeriod ? endSelectionPeriod.startDate : new Date(0));
